...
test("derives opaque session ids from common metadata field names", () => {
	...
	const fromChat = deriveApertureSessionId({ chatId: "chat-123" });
	...
	assert.match(fromChat, /^aperture-vscode-metadata-[a-f0-9]{32}$/);
	assert.notEqual(fromSession, fromConversation);
	assert.notEqual(fromConversation, fromThread);
});

test("ignores per-request identifiers when deriving session ids", () => {
	const first = deriveApertureSessionId({ requestId: "request-1" });
	const second = deriveApertureSessionId({ requestId: "request-2", parentRequestId: "parent-2" });
...
test("uses the same session id across all Aperture session headers", () => {
	const apiModes: Array<NonNullable<HFModelItem["apiMode"]>> = ["openai", "openai-responses", "anthropic", "bedrock"];
	const sessionId = deriveApertureSessionId({ sessionId: "stable-runtime-id" });

	for (const apiMode of apiModes) {
		const headers = getApertureRequestHeaders(undefined, apiMode, "aperture-copilot-extension/test", sessionId) as Record<
			string,
			string
		>;

		assert.equal(headers[APERTURE_SESSION_HEADER], sessionId);
		for (const alias of APERTURE_SESSION_HEADER_ALIASES) {
			assert.equal(headers[alias], sessionId);
		}
	}
});
